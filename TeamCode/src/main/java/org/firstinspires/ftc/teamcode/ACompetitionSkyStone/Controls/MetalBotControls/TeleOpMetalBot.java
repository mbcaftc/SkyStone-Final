...
public class TeleOpMetalBot extends OpMode {
    ...
    boolean endGame;
    int endGameTimer = 90; //number of seconds into game to signal end game.
    ...
    public void init() {
        ...
        endGame = false;
//        Bot.led_control(RevBlinkinLedDriver.BlinkinPattern.WHITE);
    }
    ...
    @Override
    public void start() {
        Bot.led_control(RevBlinkinLedDriver.BlinkinPattern.BLUE);
        TeleOpTime.reset();
        Bot.HookRelease();
        Bot.capStoneChill();
...
//        controlClawExtender();
        ...
        //moved Capstone grabbing to controlClawGrabber -jduval 2/17/20
//        controlClawGrabberCapstone();
    ...
    public void controlLEDs () {
        if (TeleOpTime.time() > endGameTimer) {
            endGame = true;
        }
        if (endGame == true) {
            Bot.led_control(RevBlinkinLedDriver.BlinkinPattern.RED);
        }
    }
...
//    }
    ...
    public void tapeMeasureControl () {
        if (gamepad2.left_stick_y < -.1) {
            telemetry.addLine("tape measuer should go out");
            // EXTEND - stick UP
            Bot.tapeMeasureMove(1);
        }
        else if (gamepad2.left_stick_y > .1) {
            telemetry.addLine("tape measuer should retract");
            // RETRACT - stick DOWN
            Bot.tapeMeasureMove(-1);
        }
        else {
            telemetry.addLine("tape measuer do nothing!");
            Bot.tapeMeasureMove(0);
        }
    ...
    public void telemetryOutput() {
        ...
        telemetry.addData("Current Time: ", TeleOpTime.time());
