...
public abstract class AutoMainWood extends LinearOpMode {
    ...
    public void driveToQuarry (WoodBot Bot, String Alliance) throws InterruptedException {
        if (Alliance == "Red") {
            Bot.driveGyroStrafe(910, .5, "right"); //was 905
            sleep(sleepTime);
//            Bot.gyroCorrection(gyroSPD, 0);
        }
    }

    public void driveGetSkyStone (WoodBot Bot, String skyLocation, String Alliance) throws InterruptedException {
        if (Alliance == "Red") {
            switch (skyLocation) {
                case "left":
                    ...
                    Bot.driveBackward(highSpeed,8.25);
                    Bot.strafeRight(highSpeed,1.25);
                    Bot.AutoClawRelease();
                    sleep(100);
                    Bot.AutoArmNorm();
                    Bot.strafeLeft(highSpeed,2);
                    Bot.driveForward(highSpeed,10.5); //10.5?
                    Bot.strafeRight(highSpeed,2.7); //was 3
                    Bot.AutoArmDown();
                    Bot.driveForward(lowSpeed,1.3);
                    Bot.AutoClawGrab();
                    sleep(500);
                    Bot.strafeLeft(highSpeed,1.5);
                    Bot.AutoArmNorm();
                    Bot.driveBackward(highSpeed,12);
                    ...
                    break;
                case "middle":
                    Bot.driveGyroStrafe(100, .5,"right");
                    break;
                case "right":
                    Bot.driveGyroStraight(200,.5,"backward");
                    break;
                default:
                    Bot.driveGyroStrafe(200,.5,"left");
                    break;
            }
            idle();
        }
